...
func TestTicketsCategorizeCmd_QueryJSON(t *testing.T) {
	...
	err := (&TicketsCategorizeCmd{QueryJSON: `{"filter":"123","query_hash":[{"condition":"status","operator":"is","value":0,"type":"default"}]}`, Page: 1, PerPage: 100}).Run(context.Background(), newTestClient(srv.URL))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
