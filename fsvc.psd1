@{
    ...
    PrivateData          = @{
        PSData = @{
            ...
            ReleaseNotes = '0.0.7 - overview rows show Elapsed (humanized business duration, e.g. "13d 14h", with minutes below a day) and Unanswered (unanswered customer messages); Since renders as RFC 3339 with the account UTC offset; HTTP calls reuse one session per base URL with the auth cookie in the session jar, identical on PowerShell 5.1 and 7 (measured ~20% faster per request, ~80 ms); the raw numeric Days property remains for sorting. 0.0.6 - overview rows get a default list view (no Id, full Link) and are grouped by category with Days descending within each group. 0.0.5 - ItildeskSession replaces SessionCookie (old config key still read); non-ASCII session values are rejected with the offending codepoint; HTTP errors are terminating; FSVC_* env vars override the saved config file; Set-FSvcConfig warns when called with no settings; TimeZoneId removed in favour of UtcOffset. 0.0.4 - empty-call warning and env precedence. 0.0.3 - persisted config moved to a JSON file. 0.0.2 - profile persistence. 0.0.1 - initial release.'
        }
    }
}
